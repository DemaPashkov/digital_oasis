...
if (!empty($_POST['submit'])){
    if ((!empty($_POST['surname'])) and !empty($_POST['name']) and !empty($_POST['otchestvo']) and !empty($_POST['number']) and !empty($_POST['email']) and !empty($_POST['date_birth'])  and !empty($_POST['admin'])){
        ...
        $admin=$_POST['admin'];
        mysqli_query($conn, "INSERT INTO `users` (`id`, `surname`, `name`, `otchestvo`, `number`, `email`,`date_birth`, `admin`) VALUES (NULL, '$surname', '$name', '$otchestvo','$number','$email', '$date_birth', '$admin')");
        header("Refresh:0");
    }else {
    ...
    <div class="dobaxit_danne">
        ...
        <form action="#" method="post">
            ...
            if(!empty($_GET['red_id'])){
                $query = "SELECT * FROM users where id={$_GET['red_id']}";
                $result = mysqli_query($conn, $query);
                $suppliers = mysqli_query($conn, "SELECT * FROM users" );
                for($data = []; $row = mysqli_fetch_assoc($result); $data [] = $row);
                ...
                if (!empty($_POST['update'])){
                    ...
                    $date_birth=$_POST['date_birth'];
                    $admin=$_POST['admin'];
                    mysqli_query($conn, "UPDATE `users` SET `surname` = '$surname', `name` = '$name', `otchestvo` =  '$otchestvo', `number` = '$number', `email` = '$email', `date_birth` = '$date_birth', `admin` = '$admin'  where id = {$_GET['red_id']}");
                    header("Refresh:0");
                }
            }
            ?>
            <?php
                ...
                $supplier = mysqli_query($conn, "DELETE FROM users WHERE id = {$_GET['del_id']}");        
                }   
            ?>
            </div>
        </div>
